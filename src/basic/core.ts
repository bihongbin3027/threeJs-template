...
class Core {
  ...
  async warpSpeed(data?: { el: string }) {
    ...
    // 渲染器
    const createRootCanvas = (el: string) => {
      const rootCanvas = new THREE.WebGLRenderer({
        // 消除锯齿
        antialias: true,
      });
      // 修改输出编码
      rootCanvas.outputEncoding = THREE.sRGBEncoding;
      // 启用阴影
      rootCanvas.shadowMap.enabled = true;
      if (el) {
        document.getElementById(el).appendChild(rootCanvas.domElement);
      } else {
        document.body.appendChild(rootCanvas.domElement);
      }

      return (this.rootCanvas = rootCanvas);
    };
    ...
    const createPerspectiveCamera = () => {
      ...
      const far = 5000;
      // 透视摄像机自适应
      const resizePerspectiveCameraDisplaySize = (
      ...
      resizePerspectiveCameraDisplaySize(this.rootCanvas, camera, this.scene);
    ...
    const createOrbitControls = () => {
      ...
      return (this.orbitControls = orbitControls);
    };
    ...
    return {
      ...
      scene: createScene(),
      // 创建渲染器
      rootCanvas: createRootCanvas(data && data.el),
      // 创建透视摄像机
